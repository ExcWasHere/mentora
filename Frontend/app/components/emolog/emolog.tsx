...
const EmologPage = () => {
  ...
  const fetchEmolog = async () => {
    try {
      const res = await fetch(
        `https://mentora-977901323224.asia-southeast2.run.app/api/emolog-history?user_id=${userId}`,
        {
          headers: {
  ...
  const handleSave = async () => {
    ...
    try {
      const res = await fetch("https://mentora-977901323224.asia-southeast2.run.app/api/emolog-history", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
  ...
  const handleDelete = async (id: number) => {
    if (confirm("Apakah kamu yakin ingin menghapus emosi ini?")) {
      try {
        await fetch(`https://mentora-977901323224.asia-southeast2.run.app/api/emolog/${id}`, {
          method: "DELETE",
        });
